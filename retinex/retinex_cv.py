...
def SSR(src_img, size):
    ...
    dst_Img = cv2.log(img / 255.0)      # 归一化取对数，(-∞, 0)
    dst_Lblur = cv2.log(L_blur / 255.0)
    dst_IxL = cv2.multiply(dst_Img, dst_Lblur)  # 原图和高斯模糊后的图像素点点乘
    # 计算图像的反射成分。
    log_R = cv2.subtract(dst_Img, dst_IxL)  # 提取高斯模糊前后的差异倍数，将高频的影响平滑弱化

    dst_R = cv2.normalize(log_R, None, 0, 255, cv2.NORM_MINMAX)     # 将归一化像素点变为0~255像素值
    log_uint8 = cv2.convertScaleAbs(dst_R)  # 线性变换并取绝对值
    return log_uint8
...
def MSR(img, scales):
    ...
    log_R = np.zeros((h, w, 3), dtype=np.float32)
    ...
    for i in range(scales_size):
        ...
        # 将不同尺度下的反射成分进行累积
        log_R = log_R + weight * cv2.subtract(dst_Img, dst_Ixl)
...
def MSR_my(img, scales):
    ...
    MSR = log_R / 3.0
    MSRCP_Out = cv2.convertScaleAbs(MSR)
